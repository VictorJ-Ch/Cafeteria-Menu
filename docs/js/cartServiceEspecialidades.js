...
function agregarAlCarrito(producto){
  ...
  let memoria = JSON.parse(localStorage.getItem("especiales"));
  let cantidadProductoFinal;
  //Si no hay localstorage lo creo
  if(!memoria || memoria.length === 0) {
    const nuevoProducto = getNuevoProductoParaMemoria(producto)
    localStorage.setItem("especiales",JSON.stringify([nuevoProducto]));
    actualizarNumeroCarrito();
    cantidadProductoFinal = 1;
  }
  else {
    //Si hay localstorage me fijo si el artículo ya está ahí
    const indiceProducto = memoria.findIndex(especial => especial.id === producto.id)
    ...
    localStorage.setItem("especiales",JSON.stringify(nuevaMemoria));
    actualizarNumeroCarrito();
...
function actualizarNumeroCarrito(){
  let cuenta = 0;
  const memoria = JSON.parse(localStorage.getItem("especiales"));
  if(memoria && memoria.length > 0){
