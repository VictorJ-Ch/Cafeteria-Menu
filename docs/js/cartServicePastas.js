...
function agregarAlCarrito(producto){
  ...
  let memoria = JSON.parse(localStorage.getItem("pastas"));
  ...
  if(!memoria || memoria.length === 0) {
    ...
    const indiceProducto = memoria.findIndex(pasta => pasta.id === producto.id)
    ...
    localStorage.setItem("pastas",JSON.stringify(nuevaMemoria));
    actualizarNumeroCarrito();
...
function actualizarNumeroCarrito(){
  let cuenta = 0;
  const memoria = JSON.parse(localStorage.getItem("pastas"));
  if(memoria && memoria.length > 0){
